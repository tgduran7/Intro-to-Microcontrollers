...
char buffer[31]     = " ECE3301L Sp'22 S1 \0";
char *nbr;
char *txt;
char tempC[]        = "+25";
char tempF[]        = "+77";
...
//    alarm_mode = 0;
    ...
    while (1)
    {
        ...
        if (INT1_flag == 1)
        {
            INT1_flag = 0;                      // Clear the INT1_flag
            if (ALARMEN == 1)                   // Check if ALARMEN is already set
            {
                MATCHED = 0;                    // Clear MATCHED
                ALARMEN = 0;                    // Clear ALARMEN
                color = 0;                      // Clear color
                Set_RGB_Color(0);               // Clear Set_RGB_Color()
                Deactivate_Buzzer();            // Turn off the buzzer()
                do_update_pwm(duty_cycle);      // Update with the current duty_cycle
                alarm_mode = 0;                 // Turn alarm_mode to 0
            }
            else 
            {
                ALARMEN = 1;                    // If ALARMEN is not set, turn on ALARMEN
            }
            Test_Alarm();                       // Call Test_Alarm()
        }
    }
}
...
void Test_Alarm()
{
    ...
    {
        DS3231_Turn_On_Alarm();                 // If alarm is not already on, turn on the alarm
        alarm_mode = 1;                         // Set alarm_mode to 1
    }
    else if (alarm_mode == 1 && ALARMEN == 0)
    {
        DS3231_Turn_Off_Alarm();                // Turn off alarm if it is on 
        alarm_mode = 0;                         // Clear alarm_mode
        PORTD = (PORTD & 0x8F);                 // Shift the colors for PORTD
        Deactivate_Buzzer();                    // Turn off the buzzer
    }
    else if (alarm_mode == 1 && ALARMEN == 1)
    {
        if (RTC_ALARM_NOT == 0) 
        {
            MATCHED = 1;                        // If RTC_ALARM_NOT is activated, set MATCHED
            Activate_Buzzer_4KHz();             // Activate the 4KHz buzzer
        }
        if (MATCHED == 1)
        {
            Set_RGB_Color(color++);             // Increment the color RGB
            if(color > 7) color = 0;            // Restart the cycle
            volt = read_volt();                 // Read the current voltage
            if (volt > 3)                       // Check if voltage is greater than 3
            {
                MATCHED = 0;                    // If greater, turn off MATCHED
                color = 0;                      // Clear color
                Set_RGB_Color(0);               // Clear Set_RGB_Color()
                Deactivate_Buzzer();            // Turn off the buzzer
