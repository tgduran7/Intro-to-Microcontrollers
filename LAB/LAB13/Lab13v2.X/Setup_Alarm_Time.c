...
void Increase_Alarm_Time()
{
            ...
            {
                case 0:
                    setup_alarm_hour++;                                     // Increment alarm hour
                    if (setup_alarm_hour == 24) setup_alarm_hour = 0;       // Watch for value greater than 23, then start over
                    break;

                case 1:
                    // add code for alarm_minute increment
                    setup_alarm_minute++;                                   // Increment alarm minute
                    if (setup_alarm_minute == 60) setup_alarm_minute = 0;   // Watch for value greater than 59, then start over
                    break;

                case 2:
                    // add code for alarm_second increment 
                    setup_alarm_second++;                                   // Increment alarm second
                    if (setup_alarm_second == 60) setup_alarm_second = 0;   // Watch for value greater than 59, then start over
                    break;
...
{       
            ...
            {
                case 0:
                    if (setup_alarm_hour == 0) setup_alarm_hour = 23;       // Check for value at 0
                    else --setup_alarm_hour;                                // Then decrement alarm hour
                    break;

                case 1:
                    // add code for alarm_minute decrement
                    if (setup_alarm_minute == 0) setup_alarm_minute = 59;   // Check for value at 0
                    else --setup_alarm_minute;                              // Then decrement alarm minute
                    break;

                case 2:
                    // add code for alarm_second decrement    
                    if (setup_alarm_second == 0) setup_alarm_second = 59;   // Check for value at 0
                    else --setup_alarm_second;                              // Then decrement alarm second
                    break;
...
{ 
    ...
    strcpy(txt, "ECE3301L Sp'22 S1");                                       // Text displayed 
...
void Update_Setup_Alarm_Time_Screen(void)
{
    ...
    setup_alarm_time[3]  = (setup_alarm_minute/10 % 10)  + '0';
    setup_alarm_time[4]  = (setup_alarm_minute%10)  + '0';
    setup_alarm_time[6]  = (setup_alarm_second/10 % 10)  + '0';
