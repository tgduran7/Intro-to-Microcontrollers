...
void Increase_Heater_Temp()
{
    ...
    if(setup_heater_set_temp > 120) setup_heater_set_temp = 120;            // Then check if it is greater than 120. If so, limit to 120
...
void Decrease_Heater_Temp()          
{
    ...
    if(setup_heater_set_temp < 50) setup_heater_set_temp = 50;              // Then check if it is less than 50. If so, limit to 50
    Update_Setup_Heater_Screen();                                           // Update screen with latest info
