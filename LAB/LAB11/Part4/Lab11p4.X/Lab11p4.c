...
#define D6 PORTBbits.RB7                                                                // The location of the LED D6
...
char D2[21] ={0,D2R,0,0,D2B,0,0,D2M,0,0,D2W,0,0,D2W,0,0,D2W,0,0,D2W,0};                 // Array for RGB LED Sequence D2
char D1[21] ={0,0,D1R,0,0,D1G,0,0,D1M,0,0,D1W,0,0,D1W,0,0,D1W,0,0,D1W};                 // Array for RGB LED Sequence D1
...
{ 
    ...
    ADCON1= 0x0F;                                                                       // Set ADCON1 to all Digital
    TRISA = 0x00;                                                                       // Set PORTA as Outputs
    TRISB = 0x11;                                                                       // Set PORTB bits 0 & 4 as Inputs and the rest Outputs
    TRISC = 0x00;                                                                       // Set PORTC as Outputs
    TRISD = 0x80;                                                                       // Set PORTD bit 7 as an Input and the rest as Outputs
    TRISE = 0x00;                                                                       // Set PORTE as Outputs
    RBPU=0;                                                                             // Enable the pull up resistor
    I2C_Init(100000);                                                                   // Initialize the I2C

    DS1621_Init();                                                                      // Initialize the DS1621
...
void main() 
{ 
    Do_Init();                                                                            // Call the Do_Init() function
    Initialize_Screen();
...
//    }
      ...
      DS3231_Setup_Time();                                                                // Call DS3231_Setup_Time() to write to the I2C
      while (1)                                                                           // This is for the DS3231 testing. Comment this section
      {                                                                                   // out to move on to the next section

        DS3231_Read_Time();                                                               // Read the data written to the I2C
        if(tempSecond != second)
        {
            tempSecond = second;                                                          // Assign tempSecond as second
            signed char tempC = DS1621_Read_Temp();                                       // Assign tempC from the temp value from DS1621_Read_Temp() in Celsius
            signed char tempF = (tempC * 9 / 5) + 32;                                     // Convert the temperature into Fahrenheit
            printf ("%02x:%02x:%02x %02x/%02x/%02x",hour,minute,second,month,day,year);   // Print out the time and day data written to the I2C
            printf (" Temperature = %d degreesC = %d degreesF\r\n", tempC, tempF);        // Print out the temperature data
        }
        ...
        if (nec_ok == 1)
        {
            ...
            for(int i = 0; i < 21; i++)                                                   // Search for the button
            {
                if (Nec_code1 == array1[i])
                {
                    found = i;                                                            // If the value exists, then assign found as i
                }
            }
            if (found != 0xff)                                                            // Write to the LCD based on the color and txt
            {
            ...
            printf ("NEC_Code = %x Found = %d \r\n", Nec_code1, found);                   // Print out the NEC_Code that was found
            if (found == 8)
            {
                DS3231_Setup_Time();                                                      // If the EQ button was pressed, redo the time setup
            }
            PORTD=D1[found] | D3[found];                                                  // Display the color onto PORTD if found
            PORTE=D2[found];                                                              // Display the color onto PORTE if found
            D6 = 1;                                                                       // Turn on the LED
            Activate_Buzzer();                                                            // Turn on the Buzzer
            Wait_Half_Second();                                                           // Wait for half a second
            D6 = 0;                                                                       // Turn off the lED
