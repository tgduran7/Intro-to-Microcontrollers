...
void INT0_ISR()
{
    ...
    if(MODE == 1)
    {
        NS_PED_SW = 1;                                                                      // If MODE is Day, then turn on NS flag
    }
...
void INT1_ISR()
{
    ...
    if(MODE == 1)
    {
        EW_PED_SW = 1;                                                                      // If the MODE is Day, then turn on EW flag
    }
...
void interrupt high_priority chkisr()
{
    ...
    if (INTCON3bits.INT1IF == 1) INT1_ISR();                                                // check if INT1 has occured
    if (INTCON3bits.INT2IF == 1) INT2_ISR();                                                // check if INT1 has occured
}

void Do_Flashing()
{
    char FLASHING = 1;                                                                      // Turn FLASHING on
    FLASHING_REQUEST = 0;                                                                   // Turn off FLASHING_REQUEST
    while(FLASHING == 1)
    {
        if(FLASHING_REQUEST == 1)
        {
            FLASHING_REQUEST = 0;                                                           // If FLASHING_REQUEST is pushed again, turn off FLASHING_REQUEST
            FLASHING = 0;                                                                   // Turn off FLASHING
        }
        else
        {
            Set_NS(RED);                                                                    // Turn on NS
            Set_NS_LT(RED);                                                                 // Turn on NS_LT
            Set_EW(RED);                                                                    // Turn on EW
            Set_EW_LT(RED);                                                                 // Turn on EW_LT
            Wait_One_Second();                                                              // Wait for one second
            Set_NS(OFF);                                                                    // Turn off NS
            Set_NS_LT(OFF);                                                                 // Turn off NS_LT
            Set_EW(OFF);                                                                    // Turn off EW
            Set_EW_LT(OFF);                                                                 // Turn off EW_LT
            Wait_One_Second();                                                              // Wait for one second
        }
    }
}
void main(void)
{
    RBPU = 0;                                                                               // Use the internal resistor
    Do_Init();                                                                              // Start the initialization
    ...
    while(1)                                                                                // Forever loop
    {
        ...
        if(FLASHING_REQUEST == 1)
        {
            Do_Flashing();                                                                  // If Flashing is pushed, then call Do_Flashing()
        }
    } 
}
...
{ 
    ...
    switch(direction)
    {
        case NS: NS_PED_SW = 0;                                                             // If NS_PED_SW interrupt is pressed, turn off flag
        break;                                                                              // break
        case EW: EW_PED_SW = 0;                                                             // If EW_PED_SW interrupt is pressed, turn off flag
        break;                                                                              // break
    }
}
...
void update_LCD_misc()                                                                      // Part D
{
    ...
    drawtext(87,  Switch_Txt_Y+9, SW_EWLT_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
    drawtext(112,  Switch_Txt_Y+9, SW_MODE_Txt, ST7735_WHITE, ST7735_BLACK, TS_1);
    if (FLASHING_REQUEST == 0) FlashingR_Txt[0] = '0'; else FlashingR_Txt[0] = '1';         // Update the FLASHING_REQUEST on the LCD
    if (FLASHING == 0) FlashingS_Txt[0] = '0'; else FlashingS_Txt[0] = '1';                 // Update the FLASHING on the LCD
